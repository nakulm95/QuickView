	...
	*/
	...
	function startSearch(name, since, me, meId) {
		FB.api('me/friends?limit=6000&fields=name,id', function(response) {
			...
			var totalLikes = [];
			var totalComments = [];
			var until = "<?= round($until) ?>";
			console.log(until);
			// if (since == 0) {
				var call = id + "/albums?limit=1&fields=name,photos.limit(100).since(" + since + ").until(" + until + ").fields(source,likes.limit(1000),comments.limit(1000),name,link),count";
				var limit = "<?= $limit ?>";
				getPictures(id, call, 0, 0, since, pictures, totalLikes, totalComments, name, limit, until);
			// } else
			//	getPicturesOld(id, 0, since, pictures, totalLikes, totalComments, name, 0, 0, limit, until);
		});
	}
	...
	function getRelationshipStatus(id, name) {
		FB.api((id + '?fields=relationship_status,significant_other'), function(response) {
			if (response.relationship_status) {
				...
				var to;
				if (so) {
					connector =  (status == "Married" || status == "Widowed" || status == "Separated" || status == "Divorced") ? 'to' : 'with';
				}
				var string = name + ' is ' + status.toLowerCase();
				if (connector !== null) {
					string += ' ' + connector + ' ' + so;
				}
				string += '<img src="http://www.iconarchive.com/download/i66644/designbolts/free-valentine-heart/Heart-Shadow.ico"' + 
						' style="width: 40px" class="img-circle pull-right" />';
			} else {
				var string = name + ' has no relationship status' + 
					'<img src="http://www.iconarchive.com/download/i66644/designbolts/free-valentine-heart/Heart-Shadow.ico"' + 
					' style="width: 40px" class="img-circle pull-right" />';
			}
	...
	function getLocation(id) {
		FB.api((id + '?fields=location'), function(response) {
			document.getElementById('location').innerHTML = response.location ? '<p style="margin:0;padding:0">' + response.location.name + '</p>' : "No Location Data";
		});
	}
			...
			if (dat) {
				...
				if ((dat['name'])) {
					...
					if (dat.count > 0) {
						if (dat.photos) {
							...
							var nextCall = id + "/albums?offset=" + (offset + 1) + "&limit=1&fields=name,photos.limit(100).since(" + since + ").until(" + until + ").fields(source,likes.limit(1000),comments.limit(1000),name,link),count";
				...
				if (photos) {
					for (var i = 0; i < photos.length; i++) {
						...
						pictures.push([dat2.source, likes, title, lnk]);
	...
	function getPicturesOld(id, offset, since, pictures, totalLikes, totalComments, name, index, lastIndex, limit, until) {
		var lastIndex;
		if (index == 0) {
			document.getElementById('processing').innerHTML="<h1>Processing</h1><h2>(Waiting for response from server)</h2>";
		}
		FB.api((id + "/photos/uploaded?limit=2500&since=" + since + "&until=" + until + "&offset=" + offset + "&fields=likes.limit(1000),comments.limit(1000),source,name,link"), function(response) {
			for (var i = 0; i < response.data.length; i++) {
				if (index >= limit) {
				...
				pictures.push([dat.source, likes, title, lnk]);
				for (var j = 0; j < likes; j++) {
			...
			if (response.paging && response.paging['next']) {
				getPicturesOld(id, offset + 2500, since, pictures, totalLikes, totalComments, name, index, lastIndex, limit, until);
			} else {
				processPictureData(id, since, pictures, totalLikes, totalComments, name, until);
			}
		});
	...
	function processPictureData(id, since, pictures, totalLikes, totalComments, name, until) {
		...
		row.className = "row";
		if (index == 0) {
			document.getElementById('uploadedPics').innerHTML += '<h3>No pictures in timeframe given</h3>';
		}
		for (var i = 0; i < index; i++) {
	...
	function getTagged(id, since, name) {
		FB.api((id + "/photos/tagged?limit=400&fields=likes.limit(10000),source&since=" + since), function(response) {
			...
			for (var i = 0; i < response.data.length; i++) {
				var likes;
				var dat = response.data[i];
				likes = dat.likes ? dat.likes.data.length : 0;
				taggedPictures[i] = [dat.source, likes];
			}
			...
			var index = Math.min(3, taggedPictures.length);
	...
	function getStatuses(id, since, totalLikes, totalComments, name, pictures, until) {
		...
		FB.api((id + "/statuses?limit=10000&fields=comments.limit(10000),from,likes.limit(10000),message&since=" + since + "&until=" + until), function(response) {
			...
			row.className = "row";
			if (index == 0) {
				document.getElementById('statuses').innerHTML += '<h3>No statuses in the timeframe given</h3>';
			}
			for (var i = 0; i < index; i++) {
				var like = "likes";
				if (statuses[i][1] == 1) {
	...
	function getFinalInfo(id, totalLikes, totalComments, since, pictures, statuses) {
		...
		row.className = "row";
		if (index == 0) {
			document.getElementById('overall').innerHTML += '<h3>No friend activity in timeframe given</h3>';
		}
		for (var i = 0; i < index; i++) {
			var com = "comments";
			var like = "likes";
			...
			row.innerHTML += '<div class="container"><div class="heading"><h2><img class="friendPic" id="friend_' + i + '" style="display: none" src="https://graph.facebook.com/' + total[i][4] + '/picture?width=' + width + '" /><a href="https://facebook.com/' + total[i][4] + '/" target="_blank">' + total[i][0] + '</a>' +  
...
function sendFinalData(pics, likes, comments, likesAndComments, topStatuses) {
	...
	moreFriends();
	moreComments();
	document.getElementById('from').value="<?= (isset($_POST['from']) && strlen($_POST['from']) > 0 ?  $since / 31556926 + 1970 : '') ?>";
	document.getElementById('until').value="<?= (isset($_POST['until']) && strlen($_POST['until']) > 0 ? $until / 31556926 + 1969 : '') ?>";
	document.getElementById('tags').value="<?= $name ?>";
}
...
function moreFriends() {
	...
	for (var i = 0; i < 3; i++) {
		...
		if (totFIndex < total.length) {
			...
			row.innerHTML += '<div class="container"><div class="heading"><h2><img class="friendPic" id="friend_' + totFIndex + 
...
function lessFriends() {
	...
	if (totFIndex > 4) {
		document.getElementById('overall').removeChild(document.getElementById('overall').lastChild);
		var child = document.getElementById('overall').lastChild;
...
<div id="all">
    <div id="banner">
        <a href="index.php"><img id="binoc" src="banner_binoc.png" alt="QuickView"/></a><!--<img id="settingsImg" src="settings.png" onclick="settingsTab()">--><br />
    </div>
    <div id="wrapper">
