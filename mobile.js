			...
			function startSearch(name, me, meId) {
				FB.api('me/friends?limit=6000&fields=name,id', function(response) {
					...
					if (!foundName) {
						...
						e("header").innerHTML='<p>Sorry, it looks like ' + getName() + ' is not a Facebook friend.</p>';
						 return;
					}
						...
						if ((dat.name)) {
							if (dat.count > 0) {
								...
								// result in pictures being added multiple times
									...
									var nextCall = id + "/albums?offset=" + (offset + 1) + "&limit=1&fields=name,photos.limit(100).since(" + since + ").until(" + until + ").fields(iamges,source,likes.limit(1000),comments.limit(1000),name,link),count";
