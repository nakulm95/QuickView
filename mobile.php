...
window.fbAsyncInit = function() {
	...
	function getData(since, name) {
		...
		*/
		...
		var name = "<?= htmlspecialchars(html_entity_decode($name)) ?>";
	...
	function processPictureData(id, since, pictures, totalLikes, totalComments, name, until) {
		...
		for (var i = 0; i < index; i++) {
			...
			document.getElementById('uploadedPics').innerHTML += '<div class="container">' + 
																	'<img src="' + pictures[i][0] + '" alt="Popular Picture" />' + quote +
	...
	function getStatuses(id, since, totalLikes, totalComments, name, pictures, until) {
		...
		FB.api((id + "/statuses?limit=10000&fields=comments.limit(10000),from,likes.limit(10000),message&since=" + since + "&until=" + until), function(response) {
			...
			for (var i = 0; i < index; i++) {
				document.getElementById("statuses").innerHTML += '<div class="container"><div class="textBox"><q>' + statuses[i][0] + 
																	'</q></div><div class="caption">' + statuses[i][1] + ' likes</div><div>';
	...
	function getFinalInfo(id, totalLikes, totalComments, since, pictures, statuses) {
		...
		for (var i = 0; i < index; i++) {
			...
			document.getElementById("overall").innerHTML += '<div class="container"><div class="heading"><h2><img class="friendPic" id="friend_' + i + '" src="http://graph.facebook.com/' + total[i][4] + '/picture?width=' + width + '" /><a href="https://facebook.com/' + total[i][4] + '/" target="_blank">' + total[i][0] + '</a>' +
																		'</h2></div><div class="caption">' + total[i][2] + ' comments and ' + total[i][1] + ' likes</div></div>';
			...
			for (var i = 0; i < dat.length; i++) {
				...
				FB.api(path + '/', function(response) {
					...
					if (len == dat.length - 1) {
						...
						document.getElementById('wrapper').style.marginTop="65px";
					}
					len++;
				});
			}
...
function sendFinalData(pics, likes, comments, likesAndComments, topStatuses) {
	...
	document.getElementById('until').value="<?= (isset($_POST['until']) && strlen($_POST['until']) > 0 ? $until / 31556926 + 1970 : '') ?>";
	document.getElementById('tags').value="<?= $name ?>";
}
...
function moreStatuses() {
	...
	for (var i = 0; i < 3; i++) {
		if (statusIndex < statuses.length) {
			document.getElementById('statuses').innerHTML+='<div class="container"><div class="textBox"><q>' + statuses[statusIndex][0] + 
																	'</q></div><div class="caption">' + statuses[statusIndex][1] + ' likes</div><div>';
			statusIndex++;
		} else {
...
function moreFriends() {
	...
	for (var i = 0; i < 3; i++) {
		...
		if (totFIndex < total.length) {
			document.getElementById("overall").innerHTML += '<div class="container"><div class="heading"><h2><img class="friendPic" id="friend_' + totFIndex + '" src="http://graph.facebook.com/' + total[totFIndex][4] + '/picture?width=' + width + '" style="display:' + display + '" />' + total[totFIndex][0] + 
															'</h2></div><div class="caption">' + total[totFIndex][2] + ' comments and ' + total[totFIndex][1] + ' likes</div></div>';
			totFIndex++;
		} else {
	...
	if (totFIndexIndex < total.length)
...
<div id="wrapper">
	<div id="search" onclick="showAll()", onTouchStart="showAll()">
    	<form action="" method="post">
            ...
            <div id="submit">
                ...
                to <input id="until"  placeholder="Year (exclusive)" name="until" type="text" pattern="^20((0[4-9])|1[0-5])(\.\d*)?$"><br />
                <!--
            	<select name="since">
    ...
    <div id="main">
        ...
        <div id="friends">
            ...
            <div class="container">
                <div id="overall">
                    <h3>Overall<button onclick="showHideFriends()">Show/Hide Pictures</button></h3>
               		<hr />
                </div>
            ...
            <div class="container">
                ...
                <div id="moreLikes">
                     <button onclick="moreLikes()" id="moreLikesButton">Show More</button>
                </div>
                <div id="lessLikes">
                    <button onclick="lessLikes()" id="lessLikesButton">Show Less</button>
                </div>
            </div>
        </div>
        <div id="settings">
            ...
            <div class="container">
            	...
            	<form action="" id="maxPics" method="post">
                	<input id="limit" type="text" name="limit" pattern="\d+"/>
                    <input type="submit" />
                </form>
